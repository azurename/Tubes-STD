...
void display(dllFirma &dllF, dllKasus &dllK) {
    ...
    while (temp) {
        ...
        for (int i = 0; i < temp->KasusCount; i++) {
            cout << getKasusName(dllK, temp->Kasus[i]) << " (ID: " << temp->Kasus[i] << ") ";
        }
        cout << endl << endl;
        temp = temp->next;
    }
...
bool addKasusUsingCursor(dllFirma &dllF, dllKasus &dllK, int KasusId) {
    ...
    cout << "Kasus " << getKasusName(dllK, KasusId) << " (ID: " << KasusId << ") ditambahkan ke Firma " << dllF.cursor->name << endl;
    return true;
}
...
void deleteKasus(dllKasus &dllK, dllFirma &dllF, int KasusId) {
    ...
    adrFirma firma = dllF.head;
    while (firma) {
        for (int i = 0; i < firma->KasusCount; i++) {
            if (firma->Kasus[i] == KasusId) {
                for (int j = i; j < firma->KasusCount - 1; j++) {
                    firma->Kasus[j] = firma->Kasus[j + 1];
                }
                firma->KasusCount--;
                break;
            }
    ...
    if (found) {
        cout << "Kasus dengan ID " << KasusId << " berhasil dihapus." << endl;
    } else {
        cout << "Kasus dengan ID " << KasusId << " tidak ditemukan." << endl;
    }
}
...
void removeKasusFromCursor(dllFirma &dllF, int KasusId) {
    ...
    if (found) {
        cout << "Kasus berhasil dihapus dari Firma." << endl;
    } else {
        cout << "Kasus tidak ditemukan dalam Firma." << endl;
    }
}
